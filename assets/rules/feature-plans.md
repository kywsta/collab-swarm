...
- A ticket's front matter carries only `title`, `status`, `depends_on`, and `skills`; `skills` names only ticket-role skills, which `npx swarm packs` lists.
...
- Run `npx swarm validate` after changing any of these files.
